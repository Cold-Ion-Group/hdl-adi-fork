...
- Synthesis/implementation: routed implementation completed, then bitstream generation failed on PG203 license.
...
- Methodology: PASS with warnings only.
  - `phase_e_logs/phase_e_methodology.rpt`: no errors or critical warnings. Remaining warnings are mostly PG203/debug-hub reset/CDC methodology warnings and existing timing-exception warnings.
- Bitstream/XSA: BLOCKED.
  - Missing `awg_kcu116.runs/impl_1/system_top.bit` and `awg_kcu116.sdk/system_top.xsa` because PG203 bitstream generation is not licensed on this installation.
