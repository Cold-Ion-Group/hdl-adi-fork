...
- **TIM‑2b (late-event policy, v1):** For late commits, acceptance policy is **commit immediately** while recording telemetry (`late_event`, `late_event_seen`, `late_event_count`, `last_late_event_id`). A halt-on-late mode is deferred to a later revision.
...
| TIM‑2 | Agility latency (freq/phase/amp hop) + commit-time classification | Insert marker (time‑domain) and command update; capture time‑to‑settle and any transients. Explicitly run early/due/late commit vectors and read scheduler telemetry registers. | MUST: report p50/p95/p99/max latency; MUST show expected behavior for early/due/late classes; for late requests, MUST commit immediately and increment `late_event_count` while updating `last_late_event_id`. | `latency_stats.csv`, `commit_timing_validation.csv`, waveforms, register dumps |
